      ...
      <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16 space-y-4">
          <h2 class="text-5xl font-black tracking-tighter uppercase sm:text-6xl">
            Domestic <span class="text-primary italic">Wonders</span>
          </h2>
          <p class="text-gray-400 text-lg max-w-2xl mx-auto font-medium">
            Explore the beauty within India
          </p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          <app-trip-card *ngFor="let trip of domesticTrips" [trip]="trip"></app-trip-card>
        </div>

        <div class="flex justify-center mt-12">
          <button routerLink="/domestic" 
            ...
            <span>View All Domestic Trips</span>
...
export class DomesticSectionComponent implements OnInit {
  domesticTrips: Trip[] = [];

  constructor(private tripService: TripService) {}

  ngOnInit(): void {
    this.tripService.getTripsByType('domestic').pipe(take(1)).subscribe(trips => {
      this.domesticTrips = trips.slice(0, 3);
    });
  }
}
